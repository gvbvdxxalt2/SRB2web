...
emcmake cmake .. -DCMAKE_BUILD_TYPE=Release \
    ...
    "-DCMAKE_EXE_LINKER_FLAGS=-DEMSCRIPTEN -s MODULARIZE=1 -s EXPORT_NAME='createSRB2Module' -s NODERAWFS=1 -s STACK_SIZE=5242880 -s INITIAL_MEMORY=268435456 -s ALLOW_TABLE_GROWTH=1 -s USE_SDL=2 -s USE_SDL_IMAGE=2 -s USE_SDL_MIXER=2 -s USE_SDL_TTF=2 -s USE_ZLIB=1 -s USE_LIBPNG=1 -s ALLOW_MEMORY_GROWTH=1 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','UTF8ToString','stringToUTF8','lengthBytesUTF8','allocate','intArrayFromString','stackAlloc','callMain'] -s ENVIRONMENT=node -L$VCPKG_LIB" \
